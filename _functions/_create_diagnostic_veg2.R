...
create_diagnostic_veg <- function(veg.dat, su, minimportance = 0.5, minconstancy = 50, noiseconstancy = 25,
  ...
  ### Differential = square root of constancy with max value of 4
  vegsum <- vegsum %>% mutate(d.potential = ifelse(constant_type %in% c("c","cd"), ((Constancy/100)*4),
                                                   ifelse(constant_type %in% c("cm"), ((Constancy/100)*2),0))) 
  vegsum <- vegsum %>% mutate(d.potential = ifelse(d.potential <0.67, 0, d.potential))
  ### dominant differential = cube root of Mean cover with max value of 4
  vegsum <- vegsum %>% mutate(dd.potential = ifelse(constant_type == "cd", ((MeanCov^covadj)), 0)) 
